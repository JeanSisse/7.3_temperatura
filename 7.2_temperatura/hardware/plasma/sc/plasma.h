...
SC_MODULE(plasma) {
	...
	bool 				clock_aux;
	bool				stop_execution;	// MDO: INCERIDO 19/10
	...
	char aux[255];
	char aux2[255]; //VAR AUX PARA PRINT NO TERMINARL
	FILE *fp;
	...
	sc_module(name_), router_address(address_), logfile(logfile_), is_master(is_master_)
	{
		...
		router->total_flits_o(total_flits);
