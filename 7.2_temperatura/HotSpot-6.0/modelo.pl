...
open( FILE_PRE_MODELO, "<$path_pre_modelo\/coeficientes_$g.txt" ) or die("Could not open file $_[0].");
my @entrada_pre_modelo = <FILE_PRE_MODELO>;
close(FILE_PRE_MODELO);
$size_pre_modelo = @entrada_pre_modelo;



	for($i=0;$i<$size_pre_modelo-1;$i++){
		$entrada_pre_modelo[$i]=~ s/\n//ig;
		@aux = split(/\t/,$entrada_pre_modelo[$i]);
		
		for($j=0;$j<9;$j++){
			$power_pre_modelo[$g][$i][$j]=$aux[$j];
			print "$power_pre_modelo[$g][$i][$j] gui \n"; 
		}
	}
				...
				for($j=1;$j<$size_pre_modelo;$j++){
					for($t=0;$t<$n_processor;$t++){
						...
						if(($y_aux == $y_atual-1) and ($x_aux == $x_atual-1)){						
							$temp_modelo[($j-1)+($i-1)][$t] = (($power_entrada_lines[$g]/$max_power) * $power_pre_modelo[$g][$j-1][0]) + $temp_modelo[($j-1)+($i-1)][$t];
																									#  6  7  8	
						}																			#  3  4  5
																									# |0| 1  2
						
						if(($y_aux == $y_atual-1) and ($x_aux == $x_atual)){						
							$temp_modelo[($j-1)+($i-1)][$t] = (($power_entrada_lines[$g]/$max_power) * $power_pre_modelo[$g][$j-1][1]) + $temp_modelo[($j-1)+($i-1)][$t];
						}																			#  6  7  8
						                                                                            #  3  4  5
						                                                                            #  0 |1| 2

						if(($y_aux == $y_atual-1) and ($x_aux == $x_atual+1)){						
							$temp_modelo[($j-1)+($i-1)][$t] = (($power_entrada_lines[$g]/$max_power) * $power_pre_modelo[$g][$j-1][2]) + $temp_modelo[($j-1)+($i-1)][$t];
						}																			#  6  7  8
						                                                                            #  3  4  5	
                                                                                                    #  0  1 |2|
                                                                                                    
						if(($y_aux == $y_atual) and ($x_aux == $x_atual-1)){						
							$temp_modelo[($j-1)+($i-1)][$t] = (($power_entrada_lines[$g]/$max_power) * $power_pre_modelo[$g][$j-1][3]) + $temp_modelo[($j-1)+($i-1)][$t];
							print "merda $g $power_pre_modelo[$g][$g][$j-1][3]\n";
						...
						if(($y_aux == $y_atual) and ($x_aux == $x_atual)){																					
							$temp_modelo[($j-1)+($i-1)][$t] = (($power_entrada_lines[$g]/$max_power) * $power_pre_modelo[$g][$j-1][4]) + $temp_modelo[($j-1)+($i-1)][$t];
						...
						if(($y_aux == $y_atual) and ($x_aux == $x_atual+1)){						
							$temp_modelo[($j-1)+($i-1)][$t] = (($power_entrada_lines[$g]/$max_power) * $power_pre_modelo[$g][$j-1][5]) + $temp_modelo[($j-1)+($i-1)][$t];
						}                                                                           #  6  7  8
                                                                                                    #  3  4 |5|
                                                                                                    #  0  1  2
						...
						if(($y_aux == $y_atual+1) and ($x_aux == $x_atual-1)){						
							$temp_modelo[($j-1)+($i-1)][$t] = (($power_entrada_lines[$g]/$max_power) * $power_pre_modelo[$g][$j-1][6]) + $temp_modelo[($j-1)+($i-1)][$t];
						}                                                                           # |6| 7  8
					                                                                                #  3  4  5
					                                                                                #  0  1  2
					                                                                                					                                                                               
						if(($y_aux == $y_atual+1) and ($x_aux == $x_atual)){						
							$temp_modelo[($j-1)+($i-1)][$t] = (($power_entrada_lines[$g]/$max_power) * $power_pre_modelo[$g][$j-1][7]) + $temp_modelo[($j-1)+($i-1)][$t];
						}                                                                           #  6 |7| 8
                                                                                                    #  3  4  5
						                                                                            #  0  1  2
						...
						if(($y_aux == $y_atual+1) and ($x_aux == $x_atual+1)){						
							$temp_modelo[($j-1)+($i-1)][$t] = (($power_entrada_lines[$g]/$max_power) * $power_pre_modelo[$g][$j-1][8]) + $temp_modelo[($j-1)+($i-1)][$t];
						}                                                                           #  6  7 |8|
		                                                                                            #  3  4  5
						                                                                            #  0  1  2
