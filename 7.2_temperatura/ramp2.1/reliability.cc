...
extern double T_base; 							/* Qualification temperature (intialized in reliability.h)*/
...
void UnitRel::init(floorplan_structure *floorplan,int unit_number)
{
	...
	rel_unit = &floorplan_structure->units[unit_number];		 /* The floorplan file is used for each structure*/
	...
	*/
	...
	printf("The area of structure %s is %f \n",rel_unit->name,unit_area);
} /* UnitRel::init*/		
