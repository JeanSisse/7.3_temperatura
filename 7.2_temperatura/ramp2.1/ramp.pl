use POSIX; 
	...
	$TOTAL_EM_FITS 		= 2283;
	$TOTAL_SM_FITS 		= 2283;
	$TOTAL_TDDB_FITS	= 2283;
	$TOTAL_TC_FITS 		= 2283;
	$TOTAL_NBTI_FITS 	= 2283;

	#$T_base 			= 72 + 273.15; # ARRUMAR #Qualification temperature. At this value, the total processor FIT value will be 4000. By changing this temperature, we can model processors with different reliability qualification costs (Higher implies more expensive)

	$EM_Ea_div_k  		= (0.9/(8.62e-5)); 	#EM activation energy [1]
	
	$SM_Ea_div_k 		= (0.9/(8.62e-5)); 	#/* SM activation energy [1]*/

	$SM_T_base			= 226.85 + 273.15;		#/* Stress free temperature or metal deposition temperature (assuming sputtering) [1]*/
	#$SM_T_base			= 226.85 + 273.15;		#/* Stress free temperature or metal deposition temperature (assuming sputtering) [1]*/
	...
	for($i=1;$i<$size_temp;$i++){
		...
		for($unitc=0; $unitc<$total_structures; $unitc++){
			$temperature[$i-1][$unitc] = $aux[$unitc] + 273.15;
		}
	}
	...
	for($unitc=0; $unitc<$total_structures; $unitc++){
		...
		$mean_temp[$unitc] = 319.15; 
		$T_base[$unitc] = 46.6 + 273.15; 
	...
	for($idx_temp=0; $idx_temp < $size_temp; $idx_temp++){
		...
		for($structures=0; $structures<$total_structures; $structures++){
			...
			$temp_diff 	= (1/$T_base[$structures]) - (1/$temperature[$idx_temp][$structures]); 						# Temperature difference between base and current sample
			#$temp_diff 	= (1/338) - (1/$temperature[$idx_temp][$structures]); 						# Temperature difference between base and current sample
			...
			$temp_ratio = ($SM_T_base - $temperature[$idx_temp][$structures])/($SM_T_base - $T_base[$structures]);
			$temp_diff 	= (1/$T_base[$structures]) - (1/$temperature[$idx_temp][$structures]);
			...
			$base_exp = pow(2.718,((0.759/((8.62e-5)*$T_base[$structures]))-(66.8/((8.62e-5)*pow($T_base[$structures],2.0)))-9.7099));
			...
			$temp_ratio = $T_base[$structures]/$temperature[$idx_temp][$structures];
			$temp_diff = (1.0/$T_base[$structures]) - (1.0/$temperature[$idx_temp][$structures]);
			$rel_exp = 795.2*$temp_diff;

			$NBTI_basemttf = pow(((log(1.6368/(1.0+2.0*pow(2.7182818,(856.1067/$T_base[$structures])))) - log((1.6368/(1.0+2.0*pow(2.7182818,(856.1067/$T_base[$structures]))))-0.01))*($T_base[$structures]/(pow(2.7182818,(-795.2/$T_base[$structures]))))),(1.0/$NBTI_beta));
			...
			#$T_base[$structures] = $temperature[$idx_temp][$structures];
